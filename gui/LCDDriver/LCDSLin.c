...
//#define XY2OFF(x,y) ((x>>3)+y*((LCD_XSIZE_PHYS+7)>>3))+(LCD_Layer-1)*9600  //不同的显示区，地址不同
...
//#define GSTART (0x0)
#define GSTART (9600)		// //JIM LEE修改  第2区首地址

#if (LCD_YSIZE_PHYS >128)
 // #define TSTART 30000
  #define TSTART 0x0  //JIM LEE修改  第1区首地址
...
static void LCD_SetScroll(int Adr) {
  ...
  LCD_WRITEDATA(LCD_YSIZE_PHYS);//JIM LEE修改， 防止最下面一行最后一个字节不显示
...
void LCD_L0_ReInit(void) {
  ...
  //LCD_WRITEDATA(1);
  LCD_WRITEDATA(0x1d);		//JIM LEE修改  1 3区图形方式 二重合成 异或逻辑
...
extern void Setbyte(int x, int y, U8 c){	// 按字节画点函数 X,Y都以点为单位
	 ...
	 for(i=0;i<8;i++){
		...
		_SetPixel((x)+i,y,LCD_COLORINDEX);     //LCD  前景色画点  较快
			      }
		else
		_SetPixel((x)+i,y,LCD_BKCOLORINDEX);     //LCD  背景色画点  较快
		c<<=1;	   
		             }
		                              }
...
extern void Setbit_nom(int x, int y){    // 按字节画点函数 X8个点(一个字节)为单位,Y以点为单位,X起点为任意位置(j为起点位置X坐标的偏移量)
         						_SetPixel(x,y,LCD_COLORINDEX);     //LCD  前景色画点  较快
                                                   }
